...
class DataCleaner:
    ...
    def load_data(self):
        ...
        # Convert CustomerID to 6-character format
        self.df["CustomerID"] = (
            self.df["CustomerID"]
            .astype(str)
    ...
    def clean_data(self):
        ...
        # Remove products with invalid quantity or price
...
class FeatureEngineer:
    ...
    def create_customer_features(self):
        ...
        print("Calculating features for each customer...")
        ...
        print("✓ Completed feature calculation!")
    ...
    def transform_features(self):
        ...
        feature_values = customer_features_indexed.values + 1  # Add 1 for Box-Cox
    ...
    def save_features(self, output_dir="../data/processed"):
        ...
        print(f"✓ Saved all features to: {output_dir}")
...
class ClusterAnalyzer:
    ...
    def create_radar_chart(self, k, cluster_names=None):
        ...
        important_features = {
            ...
            "Sum_TotalPrice": "Total Spending",
            "Mean_UnitPrice": "Preferred Price",
            "Count_Invoice": "Purchase Frequency",
            "Count_Stock": "Product Diversity",
            "Mean_TotalPriceSumPerInvoice": "Value/Transaction",
        }
        ...
        if not cluster_names:
            cluster_names = [f"Group {i}" for i in range(k)]
        ...
        for idx, (cluster_id, row) in enumerate(data_normalized.iterrows()):
            ...
            cluster_name = (
                cluster_names[idx] if idx < len(cluster_names) else f"Group {idx}"
            )
    ...
    def create_individual_radar_plots(self, k, cluster_names=None):
        ...
        important_features = {
            ...
            "Count_Stock": "Product Diversity",
            "Mean_TotalPriceSumPerInvoice": "Value/Transaction",
            "Mean_TotalPriceMeanPerStock": "Spending/Product",
            "Mean_StockCountPerInvoice": "Products/Transaction",
        }
        ...
        # Create subplot for each cluster with optimized layout
        if k == 4:
            # 2x2 layout for k=4
            nrows, ncols = 2, 2
            figsize = (12, 10)
        else:
            # 1 row layout for other cases
            nrows, ncols = 1, k
            figsize = (5 * k, 5)
        ...
        # Ensure axes is always a 2D array for easier processing
        if k == 1:
            axes = np.array([[axes]])
        elif k == 4:
        ...
        plt.suptitle(
            f"Detailed Cluster Analysis (K={k})", size=16, weight="bold", y=1.05
        )
        plt.tight_layout()
        plt.show()
    ...
    def train_surrogate_model(self, k):
        """
        Train a RandomForest classifier to simulate the KMeans algorithm.
        This model will be used for SHAP explanation analysis.
        ...
        # Get all columns that are not Cluster_
    ...
    def calculate_shap_values(self, k):
        """
        Calculate SHAP values for clustering result explanation using all data.
        ...
        # Create SHAP explainer with all data as background
        # The larger the background data, the more accurate the SHAP algorithm
        print(f"Calculating SHAP values for {len(X):,} customers...")
        explainer = shap.TreeExplainer(rf_model)
        shap_values_raw = explainer.shap_values(X)

        # Convert to list format for multiclass case
        ...
        if isinstance(shap_values_raw, np.ndarray) and len(shap_values_raw.shape) == 3:
            # Multiclass case: transpose to get (n_classes, n_samples, n_features)
            shap_values = [shap_values_raw[:, :, i] for i in range(shap_values_raw.shape[2])]
        else:
            # Binary case: Already in list format or binary classification
        ...
        print(f"Complete! SHAP values: {len(shap_values)} clusters, each cluster shape: {shap_values[0].shape}")
    ...
    def save_clusters(self, output_dir="../data/processed"):
        ...
        for k in self.cluster_results.keys():
            ...
            print(
                f"Saved clustering results k={k}: {output_dir}/customer_clusters_k{k}.csv"
            )
...
class DataVisualizer:
    ...
    def plot_revenue_over_time(self, df):
        ...
        # Monthly revenue
        plt.figure(figsize=(12, 5))
        monthly_revenue = df.groupby(pd.Grouper(key="InvoiceDate", freq="ME"))[
            "TotalPrice"
        ].sum()
